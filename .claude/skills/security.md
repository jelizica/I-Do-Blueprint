...
**The Supabase anon key is NOT a secret.** It's designed to be public and embedded in client applications.

| Key Type | Purpose | Can Be Public? | Bypasses RLS? |
|----------|---------|----------------|---------------|
| `anon` key | Client-side access | ✅ Yes | ❌ No |
| `service_role` key | Server-side admin | ❌ NEVER | ✅ Yes |

**Why the anon key is safe:**
1. **Row Level Security (RLS)**: All tables have RLS policies that restrict access based on the authenticated user's identity
2. **Authentication Required**: The anon key only enables connection; actual data access requires a valid JWT from Supabase Auth
3. **Rate Limiting**: Supabase applies rate limits to prevent abuse
4. **No Elevated Privileges**: The anon key cannot bypass RLS or access other users' data
...
// static let supabaseServiceKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```
