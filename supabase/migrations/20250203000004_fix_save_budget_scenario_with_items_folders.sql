-- Fix save_budget_scenario_with_items RPC to include folder fields
-- This ensures folders are properly saved with their folder-specific attributes
...
BEGIN
    ...
    IF p_items IS NOT NULL AND array_length(p_items, 1) > 0 THEN
        ...
        LOOP
            ...
            -- NOW INCLUDING FOLDER FIELDS: is_folder, parent_folder_id, display_order, is_expanded
...
COMMENT ON FUNCTION public.save_budget_scenario_with_items IS 'Atomically saves a budget scenario and its items. Now includes folder fields (is_folder, parent_folder_id, display_order, is_expanded) to properly support folder functionality.';
