-- Migration: Create save_budget_scenario_with_items RPC function
-- Purpose: Idempotent upsert of budget scenario and its items
-- This function handles both creates and updates without PK conflicts
-- Date: 2025-02-24
-- FIXED: Uses correct column names from budget_development_items schema

-- Drop existing function if it exists (to allow re-runs)
DROP FUNCTION IF EXISTS public.save_budget_scenario_with_items(jsonb, jsonb[]);

-- Create the function with proper upsert logic
